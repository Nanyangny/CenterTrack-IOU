...
def demo(opt):
  ...
  while True:
      ...
      if opt.dataset == "custom":
        cat_count = {'cat1':0,'cat2':0,'cat3':0,'cat4':0,'cat5':0}
      else:
        cat_count = {'cat1':0}
      for item in ret['results']:
          if item['active'] == 0:
              ...
              cat_count["cat"+str(item['class'])]+=1
      results[cnt] = cleaned_results
      cat_count.update({'total':len(cleaned_results)})
      try:
          requests.get("http://127.0.0.1:8050/api",params=cat_count)
      except:
          print("dashboard server is not running.")

      print(f'There are {len(cleaned_results)} objects identified')
