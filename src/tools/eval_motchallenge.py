...
if __name__ == '__main__':
    ...
    gtfiles = glob.glob(
      ...
      # os.path.join(args.groundtruths, '*/det/det{}.txt'.format(gt_type)))
    ...
    for k in change_fmt_list:
        fmt[k] = fmt['mota']
    eval_res =mm.io.render_summary(
      summary, formatters=fmt,
      namemap=mm.io.motchallenge_metric_names)
    print(eval_res)
    with open(args.tests + "/eval_res.txt", 'a+') as f:
        f.write(eval_res)
    if args.eval_official:
        ...
        accs, names=names, 
        metrics=metrics, generate_overall=True)
        eval_result_official = mm.io.render_summary(
            summary, formatters=mh.formatters,
            namemap=mm.io.motchallenge_metric_names)
        print(eval_result_official)
        with open(args.tests + "/eval_res.txt",'a+') as f:
            f.write("\n"+eval_result_official)
