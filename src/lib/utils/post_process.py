...
def generic_post_process(
    ...
    for i in range(len(dets['scores'])):
        ...
        for j in range(len(dets['scores'][i])):
            ...
            item['wh'] = transform_preds_with_trans(
                (dets['bboxes'][i][j][2:] - dets['bboxes'][i][j][:2]).reshape(1, 2), trans).reshape(2)
            ...
            ## tracking_ltrb bbox
            if 'tracking_bboxes' in dets:
                tracking_ltrb_amodal = transform_preds_with_trans(
                    (dets['tracking_bboxes'][i][j].reshape(2, 2)), trans).reshape(4)
                item['tracking_bboxes'] = tracking_ltrb_amodal
            ...
            if 'tracking_wh' in dets:
                if not opt.direct_wh:
                    tracking_wh = transform_preds_with_trans(
                        (dets['tracking_wh'][i][j] + (dets['bboxes'][i][j][2:]-dets['bboxes'][i][j][:2])).reshape(1, 2),
                        trans).reshape(2)
                else:
                    tracking_wh = transform_preds_with_trans(
                        (dets['tracking_wh'][i][j]).reshape(1,2), trans).reshape(2)

                item['tracking_wh'] = tracking_wh

            ## Add in iou
            if 'iou' in dets:
                item['iou'] = dets['iou'][i][j]
