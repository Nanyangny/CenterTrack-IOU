  ...
  def show_all_imgs(self, pause=False, Time=0):
    if 1:
      for i, v in self.imgs.items():
        ...
        imencoded = cv2.imencode(".jpg",v)[1].tostring()
        file ={'file':('image.jpg',imencoded,'image/jpeg')}
        data ={'id':"123"}
        try:
            requests.post("http://127.0.0.1:8050/api/img",files=file,data=data)
            print("send data")
        except:
            cv2.imwrite("../results/demo_{}.jpg".format(i), v)
