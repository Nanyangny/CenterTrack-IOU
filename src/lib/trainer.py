...
from model.losses import FastFocalLoss, RegWeightedL1Loss, IouLoss
...
class GenericLoss(torch.nn.Module):
    ...
    def forward(self, outputs, batch):
        ...
        for s in range(opt.num_stacks):
            ...
            regression_heads = [
                'reg', 'wh', 'tracking', 'tracking_wh','iou', 'ltrb', 'ltrb_amodal', 'hps',
                'dep', 'dim', 'amodel_offset', 'velocity','tracking_ltrb_amodal']
            ...
            ## add iou Loss to the tracking_wh and wh
            # wh_heads =['tracking_wh']
            # for head in wh_heads:
            #     if head in output:
            #         losses[head] += self.iou(
            #             output[head], batch[head + '_mask'],
            #             batch['ind'], batch[head]) / opt.num_stacks
...
class Trainer(object):
    ...
    def _get_losses(self, opt):
        loss_order = ['hm', 'wh', 'reg', 'ltrb', 'hps', 'hm_hp', \
                      ...
                      'ltrb_amodal', 'tracking', 'tracking_wh','iou', 'nuscenes_att', 'velocity','tracking_ltrb_amodal']
