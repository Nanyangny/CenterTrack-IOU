...
def check_gradient_dconv():
    ...
    offset = torch.randn(N, deformable_groups * 2 * kW * kH, inH, inW).cuda() * 2.0
    # offset.data.zero_()
    # offset.data -= 0.5
    offset.requires_grad = True

    mask = torch.rand(N, deformable_groups * 1 * kW * kH, inH, inW).cuda()*1.0
    # mask.data.zero_()
    mask.requires_grad = True
    mask = torch.sigmoid(mask)

    weight = torch.randn(outC, inC, kH, kW).cuda()*1.0
    weight.requires_grad = True

    bias = torch.rand(outC).cuda()*1.0
    ...
    print('check_gradient_dconv: ',
          gradcheck(dcn_v2_conv, (input, offset, mask, weight, bias,
                    ...
                    eps=1e-3*1.0, atol=1e-4*1.0, rtol=1e-2*1.0))
