...
def generic_decode(output, K=100, opt=None):
  ...
  regression_heads = ['tracking','tracking_wh', 'iou','dep', 'rot', 'dim', 'amodel_offset',
    'nuscenes_att', 'velocity','tracking_ltrb_amodal']
  ...
  if 'tracking_ltrb_amodal' in output:
    trk_ltrb_amodal = output['tracking_ltrb_amodal']
    trk_ltrb_amodal = _tranpose_and_gather_feat(trk_ltrb_amodal, inds)  # B x K x 4
    trk_ltrb_amodal = trk_ltrb_amodal.view(batch, K, 4)
    tracking = output['tracking']
    tracking = _tranpose_and_gather_feat(tracking,inds)
    tracking = tracking.view(batch,K,2)

    if not opt.tracking_ltrb_amodal_offset:
      ## trk_ltrb_amodel + tracking
      pre_bboxes = torch.cat([trk_ltrb_amodal[..., 0:1] + tracking[..., 0:1] + xs0.view(batch, K, 1),
                                 trk_ltrb_amodal[..., 1:2] + tracking[..., 1:2] + ys0.view(batch, K, 1),
                                 trk_ltrb_amodal[..., 2:3] + tracking[...,0:1] + xs0.view(batch, K, 1),
                                 trk_ltrb_amodal[..., 3:4] + tracking[..., 1:2] + ys0.view(batch, K, 1)], dim=2)
    else:
      ## trk_ltrb_amodel + ltrb_amodal + tracking
      ltrb_amodal = output['ltrb_amodal']
      ltrb_amodal = _tranpose_and_gather_feat(ltrb_amodal, inds)  # B x K x 4
      ltrb_amodal = ltrb_amodal.view(batch, K, 4)
      pre_bboxes = torch.cat([trk_ltrb_amodal[..., 0:1] + ltrb_amodal[...,0:1]+tracking[..., 0:1] + xs0.view(batch, K, 1),
                              trk_ltrb_amodal[..., 1:2] + ltrb_amodal[...,1:2]+tracking[..., 1:2] + ys0.view(batch, K, 1),
                              trk_ltrb_amodal[..., 2:3] + ltrb_amodal[...,2:3]+tracking[..., 0:1] + xs0.view(batch, K, 1),
                              trk_ltrb_amodal[..., 3:4] + ltrb_amodal[...,3:4]+tracking[..., 1:2] + ys0.view(batch, K, 1)], dim=2)

  #
    ret['tracking_bboxes']= pre_bboxes
