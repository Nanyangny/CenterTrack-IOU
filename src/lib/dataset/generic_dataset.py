...
from utils.tracker import ious, ltbr_from_ct_and_wh
...
class GenericDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        pre_cts, track_ids, pre_whs, pre_bboxes = None,None,None,None
        if opt.tracking:
            ...
            pre_hm, pre_cts, track_ids, pre_whs, pre_bboxes, pre_bbox_amodals = self._get_pre_dets(
        ...
        ## get the ignored_regions
        for k in range(num_objs):
            ann = anns[k]
            cls_id = int(ann['category_id'])
            if cls_id > self.opt.num_classes or cls_id <= -999 or cls_id <= 0 or ('iscrowd' in ann and ann['iscrowd'] > 0):
        ...
        for k in range(num_objs):
            ...
            bbox, bbox_amodal = self._get_bbox_output(
                ann['bbox'], trans_output, height, width)
            if cls_id <= 0 or ('iscrowd' in ann and ann['iscrowd'] > 0) or cls_id > self.opt.num_classes:
                self._mask_ignore_or_crowd(ret, cls_id, bbox)
                continue
            self._add_instance(
                ret, gt_det, k, cls_id, bbox, bbox_amodal, ann, trans_output, aug_s,
                calib, pre_cts, track_ids,pre_whs,pre_bboxes,ignore_regions,pre_bbox_amodals)
    ...
    def _load_pre_data(self, video_id, frame_id, sensor_id=1):
        ...
        if 'train' in self.split:
            img_ids = [(img_info['id'], img_info['frame_id']) \
                       ...
                       (not ('sensor_id' in img_info) or img_info['sensor_id'] == sensor_id)]  ## consider nearby prior and post frames
        else:
            img_ids = [(img_info['id'], img_info['frame_id']) \
                       for img_info in img_infos \
    ...
    def _get_input(self, img, trans_input):
        ...
        inp = inp.transpose(2, 0, 1)  #bgr =>  ## rbg
        return inp
    ...
    def _init_ret(self, ret, gt_det):
        ...
        # ret['ignored_area'] = np.zeros((self.opt.output_h, self.opt.output_w),np.float32)

        regression_head_dims = {
            'reg': 2, 'wh': 2, 'tracking': 2, 'tracking_wh': 2, 'iou':1 ,'ltrb': 4, 'ltrb_amodal': 4,
            'nuscenes_att': 8, 'velocity': 3, 'hps': self.num_joints * 2,
            'dep': 1, 'dim': 3, 'amodel_offset': 2 ,'tracking_ltrb_amodal':4 }
    ...
    def _mask_ignore_or_crowd(self, ret, cls_id, bbox):
        ...
        if cls_id == 0 or cls_id > self.opt.num_classes:  # ignore all classes
    ...
    def _add_instance(
            ...
            aug_s, calib, pre_cts=None, track_ids=None, pre_whs=None, pre_bboxs=None,ignore_areas=None,pre_bbox_amodals=None):
        h, w = bbox[3] - bbox[1], bbox[2] - bbox[0]
        ct_wh = np.array([w,h],dtype=np.float32)
        if h <= 0 or w <= 0:
            return

        ct = np.array(
            [(bbox[0] + bbox[2]) / 2, (bbox[1] + bbox[3]) / 2], dtype=np.float32)
        for areas in ignore_areas:
            if (areas[0]<=ct[0] and ct[0]<=areas[2]) and (areas[1]<=ct[1] and ct[1]<=areas[3]):
                return
        radius = gaussian_radius((math.ceil(h), math.ceil(w)))
        radius = max(0, int(radius))
        ct_int = ct.astype(np.int32)
        ...
        if 'tracking_wh' in self.opt.heads:
            if ann['track_id'] in track_ids:
                ret['tracking_wh_mask'][k] = 1
                pre_wh = pre_whs[track_ids.index(ann['track_id'])]
                if self.opt.direct_wh:
                    ret['tracking_wh'][k] = pre_wh
                else:
                    ret['tracking_wh'][k] = pre_wh - ct_wh
                gt_det['tracking_wh'].append(ret['tracking_wh'][k])
            else:
                gt_det['tracking_wh'].append(np.zeros(2, np.float32))


        if 'iou' in self.opt.heads:
            if ann['track_id'] in track_ids:
                pre_ct = pre_cts[track_ids.index(ann['track_id'])]
                pre_wh = pre_whs[track_ids.index(ann['track_id'])]
                pre_ltbr =ltbr_from_ct_and_wh(pre_ct,pre_wh)
                ret['iou_mask'][k] = 1
                ret['iou'][k] = ious([pre_ltbr],[bbox])[0][0]
                gt_det['iou'].append(ret['iou'][k] )
            else:
                gt_det['iou'].append(np.zeros(1, np.float32))
        ...
        if 'tracking_ltrb_amodal' in self.opt.heads:
            if ann['track_id'] in track_ids:
                pre_ct = pre_cts[track_ids.index(ann['track_id'])]
                pre_bbox_amodal = pre_bbox_amodals[track_ids.index(ann['track_id'])]
                if not self.opt.tracking_ltrb_amodal_offset:
                    ret['tracking_ltrb_amodal'][k] = \
                        pre_bbox_amodal[0] - pre_ct[0], pre_bbox_amodal[1] - pre_ct[1], \
                        pre_bbox_amodal[2] - pre_ct[0], pre_bbox_amodal[3] - pre_ct[1]
                else:
                    ret['tracking_ltrb_amodal'][k] = \
                        pre_bbox_amodal[0] - (bbox_amodal[0] - ct_int[0]), pre_bbox_amodal[1] - (bbox_amodal[1] - ct_int[1]), \
                        pre_bbox_amodal[2] - (bbox_amodal[2] - ct_int[0]), pre_bbox_amodal[3] - (bbox_amodal[3] - ct_int[1])
                ret['tracking_ltrb_amodal_mask'][k] = 1
                gt_det['tracking_ltrb_amodal'].append(pre_bbox_amodal)
            else:
                gt_det['tracking_ltrb_amodal'].append(np.zeros(4, np.float32))
    ...
    def _format_gt_det(self, gt_det):
        if (len(gt_det['scores']) == 0):
            gt_det = {'bboxes': np.array([[0, 0, 1, 1]], dtype=np.float32),
                      ...
                      'tracking_wh': np.array([[0, 0]], dtype=np.float32),
                      'iou': np.array([0], dtype=np.float32),
